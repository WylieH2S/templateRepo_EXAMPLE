...
The script detects "template mode" by the presence of `init-project.sh`. In template mode, three accommodations apply because they describe state that only becomes valid post-bootstrap: leftover `{{TOKEN}}` placeholders are skipped entirely, an unresolved `EXTENDS` path downgrades from FAIL to WARN, and substrate filenames resolve through their placeholder variants (`NAME.chloeai` ⇄ `NAME.{{AI}}ai`, `WORKSHEET.heywy` ⇄ `WORKSHEET.hey{{HUMAN}}`). The filename resolution is what lets the template itself validate 0-fails and serve as the boot-contract conformance oracle.
