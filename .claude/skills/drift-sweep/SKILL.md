...
5. **Orphaned exports** — for each `export function|const|class|let|var|interface|type|enum NAME` under `CODE_ROOTS`, count `\bNAME\b` references in other source files. Zero hits = FAIL. Skips `index.*` and `mod.d.ts` files; honors `ORPHAN_EXPORT_ALLOWLIST` regex.
...
7. **CLAUDE.md path-rules table sanity** — verifies `CLAUDE.md` exists and contains a path-scoped rules table header.
...
- **v0.1 (current)** — initial implementation; check categories above. Comment-claim mismatch check (e.g., comment says `lock=9999` but code uses `60`) is documented but not implemented — too noisy for v0.1.
