...
3. **Known-cruft directories + versioned backups** — tracked files under `*/old/`, `*/testing/`, `*_backup*/`, `*_TEMP*/`; files matching `*_v[0-9]+(_[0-9]+)*.{ts,py,etc}` outside `tests/` or `spec/`. Honors `VERSIONED_BACKUP_ALLOWLIST` (v0.1.20), a path regex for files whose version **is the meaning** — a pinned snapshot of an external surface rather than a stale copy.
