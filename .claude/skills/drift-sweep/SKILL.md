...
9. **WISL waystone freshness** — for each `_waystone.chloeai` in the repo: parse `verified_at` (quote-tolerant) and `owns` globs; FAIL if `verified_at` is unparseable or dangling, or if any owned file changed since `verified_at` — **excluding the waystone file itself** (so re-stamping the waystone doesn't self-trip the gate, WISL pilot friction #1). No `_waystone.chloeai` present → graceful PASS (WISL not adopted in this repo). Gateable via `--fail-on=waystone-freshness`. Implements the WISL-STANDARD §Enforcement `owns`↔`verified_at` binding — the up-sync "teeth" that turn "touched a folder → must re-stamp its waystone" into a CI failure.
