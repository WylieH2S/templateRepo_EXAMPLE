...
# <ext> is RESOLVED from that root's own substrate since 2026-08-16:
#   readme_AI.<ext>                          present → in-repo session
#   .claude/skills/hi-mode/HANDOFF_LOG.<ext> else    → workspace session
...
# Previously this script looked for `./readme_AI.<ext>` relative to CWD alone.
...
# The carrier extension is RESOLVED per candidate root, not hardcoded — same
# mechanism as drift-sweep and validate-substrate (see the long note at
# drift-sweep's resolver). Per-root because the candidates can be different
# repos, and the suffix is a property of the repo, not of the machine.
_carrier_ext() {
  local _r="$1" _stem _f _s
  for _stem in _waystone STARTUP_AI readme_AI AI_HANDOFF SIDEQUESTS; do
    for _f in "$_r/$_stem".*; do
      { [ -e "$_f" ] || [ -L "$_f" ]; } || continue
      _s="${_f##*/}"; _s="${_s#"$_stem".}"
      case "$_s" in
...
for _root in "${CLAUDE_PROJECT_DIR:-}" "$_git_top" "$PWD"; do
  ...
  _ext=$(_carrier_ext "$_root") || continue
  if [ -f "$_root/readme_AI.${_ext}" ]; then
    ROOT="$_root"; SRC="$_root/readme_AI.${_ext}"
    CTX="in-repo: $(basename "$_root")"; break
  elif [ -f "$_root/.claude/skills/hi-mode/HANDOFF_LOG.${_ext}" ]; then
    ROOT="$_root"; SRC="$_root/.claude/skills/hi-mode/HANDOFF_LOG.${_ext}"
    CTX="workspace"; break
  fi
done
...
if [ -z "$SRC" ]; then
  ...
  echo "   Looked for readme_AI.<ext> / .claude/skills/hi-mode/HANDOFF_LOG.<ext> (ext resolved from ./_waystone.<ext>) under:"
