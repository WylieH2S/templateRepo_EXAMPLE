...
4. **Drift findings:** `.repo-manager/drift_log.{{AI}}ai` — fleet drift state and OPEN DUE-OUTS ledger. Read when investigating drift or after a sweep.
5. **Tooling intelligence (if present):** `.claude/skills/hi-mode/ski_lift_log.{{AI}}ai` — read at STRATEGIC zoom; skip at OPERATIONAL.
...
- Cross-repo drift findings go in `.repo-manager/drift_log.{{AI}}ai` (append-only journal).
